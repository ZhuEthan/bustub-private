...
auto UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  while (child_executor_->Next(tuple, rid)) {
    ...
    for (auto &x : index_infos_) {
      Tuple partial_tuple = tuple->KeyFromTuple(table_info_->schema_, *(x->index_->GetKeySchema()), x->index_->GetKeyAttrs());
      x->index_->DeleteEntry(partial_tuple, *rid, exec_ctx_->GetTransaction());
      auto iwr1 = IndexWriteRecord{*rid, table_info_->oid_, WType::DELETE, partial_tuple, x->index_oid_, exec_ctx_->GetCatalog()};
      exec_ctx_->GetTransaction()->GetIndexWriteSet()->push_back(iwr1);


      Tuple partial_new_tuple = new_tuple.KeyFromTuple(table_info_->schema_, *(x->index_->GetKeySchema()), x->index_->GetKeyAttrs());
      x->index_->InsertEntry(partial_new_tuple, *rid, exec_ctx_->GetTransaction());

      auto iwr2 = IndexWriteRecord{*rid, table_info_->oid_, WType::INSERT, partial_new_tuple, x->index_oid_, exec_ctx_->GetCatalog()};
