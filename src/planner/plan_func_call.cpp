...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception(fmt::format("func call {}'s args's length should be 1", func_name));
  }
  return std::make_shared<StringExpression>(
      args[0], func_name == "lower" ? bustub::StringExpressionType::Lower : bustub::StringExpressionType::Upper);
